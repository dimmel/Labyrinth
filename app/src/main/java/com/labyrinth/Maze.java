...
public class Maze implements Drawable {
    ...
    private void generateMaze() {
        ...
        while (stack.size() > 0) {
            ...
            if (unusedNeighbors.size() > 0){
                ...
                Point dirrection = unusedNeighbors.get(rnd);
                int diffX = (dirrection.x - current.x) / 2;
                int diffy = (dirrection.y - current.y) / 2;
                array[current.y + diffy][current.x + diffX] = true;
                stack.push(dirrection);
            } else {
    ...
    public void draw(Canvas canvas, Rect rect) {
        ...
        Log.e("MAZE", String.valueOf(cellSize));
        for (int i=0; i<size; ++i){
            for (int j=0;j<size;++j){
                if(!array[i][j]){
